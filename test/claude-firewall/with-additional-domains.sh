...
# Check if we can reach google.com (should be allowed via allowedDomains)
if curl --connect-timeout 5 https://google.com >/dev/null 2>&1; then
    check "can reach google.com (custom allowed domain)" true
else
    echo "ERROR: Cannot reach google.com, which should be allowed via allowedDomains"
    check "can reach google.com (custom allowed domain)" false
fi
